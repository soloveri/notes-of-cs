...
上面的配图果然是错误的，对于符合索引，一个索引节点包含所有的列，例如复合索引（A，B，C）。只有当A相等时才会比较B，B相等时比较C。所以如果直接跳过A，是无法直接使用这个索引的。再偷一张图:![复合](https://user-gold-cdn.xitu.io/2020/2/27/170867eb79a354d0?imageView2/0/w/1280/h/960/format/webp/ignore-error/1)

可以看出只有第一行相等时，第二行才保持有序，第二行相等时，第三行才保持有序。

所以对于一个复合索引来说，一个索引节点是包含所有的列，而不是单单仅包含第一个列。[参考](https://juejin.cn/post/6844904073955639304)
