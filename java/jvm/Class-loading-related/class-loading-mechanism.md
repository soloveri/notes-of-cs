...
初始化一定要求类或者接口被加载，但是反之则不一定啊，加载了类或接口，不一定需要初始化。所以我猜测，当是一个实现类在解析接口引用时，只是加载了接口，而没有初始化接口。不然类怎么实现验证步骤？
...
在Java中，只有主动引用才会触发~~类~~类型加载动作。所谓的主动引用指**只**包括以下~~五~~六种情况（除了下面这六种情况，其余的引用都称为被动引用）：
