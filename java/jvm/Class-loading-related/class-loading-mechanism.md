...
1. 如果c不是一个数组类型，那么会使用D的类加载器加载c，其中会将c的全限定名传递给D的类加载器。如果其中又需要加载其他类，那么重复整个解析程序
...
1. 收集类中所有类变量的赋值动作和静态代码块的语句按顺序而合成，如果没有静态变量赋值语句和静态代码块，那么并不会生成`<cinit>()`

2. 静态代码块只能访问定义在块之前的静态变量，对于块之后定义的静态变量，静态代码块只能赋值，不能访问

3. 虚拟机会保证父类的类构造器`<cinit>()`一定会在子类的`<cinit>()`执行之前执行，这与实例构造器`<init>()`一样，会保证父类的实例构造器先于字类执行
...
**前四条的方法调用逻辑**是完全固化在虚拟机内的，而最后一条指令的调用指派完全由程序员指定。
...
这类方法的调用称为**分派**。顾名思义，分派肯定是从多个候选者选择一个。我认为这是与解析最大的不同，因为解析调用的候选方法只有一个。分派分细分为静态分派和动态分派。

#### 7.2.1 静态分派

所谓的静态分派和上述的解析并没有太大差别，都是在编译期确定目标方法的符号引用。值得注意的是：在重载环境下，编译器依赖方法参数的静态类型选择合适的版本。这也侧面说明了重载时只有参数不同才算重载。

这类依赖静态类型来决定目标方法版本的操作称为静态解析，因为候选者有多个！**静态解析的结果由编译器决定而不是虚拟机。**

#### 7.2.2 动态分派

静态分派是靠编译器实现的，然而动态分派是靠`invokevirtual`实现的。因为在执行方法时，局部变量的第一个槽变量永远是当前对象的引用。`invokevirtual`的执行步骤如下：

1. 找到操作数栈顶第一个元素指向的对象的实际类型C
2. 在类C中查找是否存在描述符和名称都符合目标的方法，如果存在，那么结束查找
...
对于`fahter.hardChoice(new _360())`,在编译时期编译器确定调用符号时，可以看出候选者非常多。那么在调用`father.hardChoice`时发现方法的接收者的静态类型这是`Father`,这就决定了在`Father`类中选择。那么接着查看调用方法的参数，发现实参的静态类型为`360`,那么通过方法调用者和函数实参这两个标准在编译器确定了符号调用，这就代表使用了两个宗量。同理编译`son.hardChoice(new QQ())`时也会使用两个宗量确定符号引用。
