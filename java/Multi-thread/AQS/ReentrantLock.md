...
final boolean acquireQueued(final Node node, int arg) {
    ...
    try {
        ...
        for (;;) {
            ...
            //假如此时锁被threadA占有，当前node为threadB，threadB tryAcquire失败
            //所以不用担心原子性问题
...
private static boolean shouldParkAfterFailedAcquire(Node pred, Node node) {
    ...
    if (ws > 0) {
        ...
        */
        /***********************************************
        * 为什么设置状态要使用CAS操作？什么情况下会发生竞争？此时当前node已经进入队列，谁会修改pred的status？
        * 是pred自己吗？pred的status需要靠后继节点来设置，如果它执行cancel操作后，status>0，但是下面的代码还是执行pred.status=-1，cancel的状态丢失了
        * 但是pred却不会执行获取锁的代码了，pred后面的节点永远都不能获取锁，由于pred.ws==Node.SGNAL,所以pred后面的节点永远无法跳过pred
        ***********************************************/
...
3. 当`node`为非尾节点时，并且`node`的前一个节点状态不为-1并且不能更新为-1时，那么就必须手动唤醒`node`之后的第一个有效节点。这个逻辑比较难理解。我试图从以下几张图说说我的理解。
...
**当然，需要注意上述图中所有的next指针都是在CAS操作成功的情况下决定的**
...
head的status在第一个节点入队时，也会被设置为-1，后面需要恢复。否则当队列为空时，head的status为-1，当第一个节点入队时会被阻塞

当一个线程获得锁时，它就会出队，所以不用设置它的waitStatus

//第一个条件返回true，但是此时该线程已经是第一个队列了
if (shouldParkAfterFailedAcquire(p, node) &&
                    parkAndCheckInterrupt())
                    interrupted = true;
            }